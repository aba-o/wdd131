...
function search (){
    let hikeQuerry = document.querySelector("#search").value;
    let filterHikes = hikes.filter(function(hike){
    return(
        hike.name.toLocaleLowerCase().includes(hikeQuerry.toLocaleLowerCase()) ||
        hike.description.toLocaleLowerCase().includes(hikeQuerry.toLocaleLowerCase()) ||
        hike.tags.find(tag => tag.toLocaleLowerCase().includes(hikeQuerry.toLocaleLowerCase()))
    );})
